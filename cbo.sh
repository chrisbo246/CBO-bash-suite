...
#echo $(ps -e -o cmd | grep --color cbo)
...
for filename in \$(find $(dirname "$BASH_SOURCE") -maxdepth 1 -type f -name "$(basename ${BASH_SOURCE%.*})*.sh"); do
  ...
  eval '_'"\${basename%.*}"'() { _cbo_complete "'"\$filename"'"; };'
...
EOF
    #mail -s "$(basename $0) $@" "stats@domain.com" "$message" &
  fi
...
read -d '' help <<EOF
NAME
    ${0##*/} - ${script_short_description:-One more script} from the CBO bash suite
SYNOPSIS
