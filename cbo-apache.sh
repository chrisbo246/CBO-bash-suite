...
__apache_config()
{
  apache_config=$(find / -type f -name "apache?.conf" -exec grep -li "Timeout" {} \; 2>/dev/null)
  apache_service=$( find / -type f -path "*/init.d/*" -name "apache?"  2>/dev/null)
}
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
uninstall_apache2 ()
{
...
#AddDefaultCharset UTF-8                # Jeu de caractère utilisé par défaut
  ...
  sudo sudo chmod 755 $project_path
...
move_apache_dir ()
{
  ...
  sudo sudo chmod 755 $new_document_root
...
____create_apache_vhost ()
{
  ...
  sudo sudo chmod 755 ${project_path}${vhost_name}/
  sudo sudo chmod 755 ${project_path}${vhost_name}/${web_dir}
...
select_apache_vhost()
{
  while true; do
    ...
    options=$(find /etc/apache*/sites-available/ -type f -name '*.vhost' -printf "%f${IFS}" | sed 's/.vhost//' | sort 2>/dev/null)
