...
remote_ubuntu_desktop ()
{
  ...
  # 2. Remote desktop to local port 5903 at the client (sudo which is the client's end of the SSH tunnel):
...
{
  # Note: The remote machine must be booted into Windows XP, so you may have to reboot remotely from Ubuntu to Windows first.
  # 2. If the remote machine is protected by a bastion host, you will need to create an SSH tunnel through sudo which to access the server:
  #   where
  #       * $LOCAL_PORT is a port on the local machine through sudo which you want to tunnel (e.g., 2001)
  #       * $SERVER is the hostname or IP of the remote machine
