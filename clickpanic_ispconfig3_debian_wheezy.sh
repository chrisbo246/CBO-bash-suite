...
EOF
sudo editor ${config_path}network/interfaces
...
sudo editor ${config_path}courier/imapd.cnf
set_confvar "${config_path}courier/pop3d.cnf" "CN" "$servername.$domain"
sudo editor ${config_path}courier/pop3d.cnf
...
echo "The ISPConfig 3 setup uses amavisd sudo which loads the SpamAssassin filter library internally, so we can stop SpamAssassin to free up some RAM"
...
#sudo editor ${config_path}default/pure-ftpd-common
...
maxretry = 5
EOF
sudo editor ${config_path}fail2ban/jail.local
...
ignoreregex =
EOF
sudo editor ${config_path}fail2ban/filter.d/pureftpd.conf
...
ignoreregex =
EOF
sudo editor ${config_path}fail2ban/filter.d/courierpop3.conf
...
ignoreregex =
EOF
sudo editor ${config_path}fail2ban/filter.d/courierpop3s.conf
...
ignoreregex =
EOF
sudo editor ${config_path}fail2ban/filter.d/courierimap.conf
