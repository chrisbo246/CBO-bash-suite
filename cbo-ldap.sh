...
edit_ldap_config ()
{
  ...
  sudo editor ${config_path}ldap/slapd.conf  # qui est le fichier de configuration du dÈmon slapd.
  sudo editor ${config_path}ldap/ldap.conf   # qui est le fichier de configuration des utilitaires LDAP.
  ...
  Configuration
  sudo editor ${config_path}ldap/slapd.conf   # Editer le fichier de configuration
...
9.Authoriser le protocol LDAPv2 ? non
  ...
  sudo editor ${config_path}ldap/ldap.conf
  # SÈcuriser le fichier de configuration comportant le mot de passe admin en claire
  sudo chmod 600 ${config_path}openldap/slapd.conf
...
replic_ldap ()
{
  ...
  "
  sudo editor ${config_path}ldap/slapd.conf
