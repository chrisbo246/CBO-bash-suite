...
install_samba ()
{
  ...
  sudo editor ${config_path}nsswitch.conf
...
{
  sudo editor ${config_path}nsswitch.conf
  sudo editor ${config_path}samba/smb.conf
  gksu sudo editor ${config_path}samba/smbusers
  backup_file ${config_path}fstab
  gksu sudo editor ${config_path}fstab
  # sudo pdbedit
}
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
add_samba_user ()
{
  # Now that we’ve got samba installed, let’s create a local user with sudo which people can log in with.
  ...
  sudo editor ${config_path}samba/smbusers
...
add_samba_share ()
{
  ...
  sudo sudo chmod -R 0777 "$share_dir"
