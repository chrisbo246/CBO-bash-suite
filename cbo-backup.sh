...
backup_mysql_tables()
{
  [ ! $(sudo which mysqldump) ]  >&2&& return 1
...
EOF
  ...
  # Create the restoration script
  [ ! -e 'restore.sh' ] && echo "#!/bin/bash" > restore.sh && sudo chmod u+x restore.sh
...
backup_debconf_db()
{
  [[ ! $(sudo which dpkg) ]] && return 1
  # Create the restoration script
  [ ! -e 'restore.sh' ] && echo "#!/bin/bash" > restore.sh && sudo chmod u+x restore.sh

  [[ ! $(sudo which debconf-get-selections) ]] && __package_cp -q install debconf-utils
  if [[ $(sudo which debconf-get-selections) ]]; then
    echo "Debconf backup..."
...
EOF
  ...
  find "$backup_dir" -mindepth 1 -maxdepth 1 ! -name daily ! -name weekly ! -name monthly $verbose -exec $BASH -c 'mv -f -t "'$backup_dir'/daily/"$(date --rfc-3339 date -d @$(stat -c %Z "{}")) "{}"' \;

  # Move backups of the daily directory to monthly directory if they match month day
  while read filename; do
    mv -f -t "$backup_dir/monthly/" "$filename"
  done < <(find "$backup_dir/daily/" -mindepth 1 -maxdepth 1 -type d -printf "%Td\t%p${IFS}" | grep -P '^('$monthly_day')' | cut -f 2- 2>/dev/null)

  # Move backups of the daily directory to monthly directory if they match week day
  while read filename; do
    mv -f -t "$backup_dir/weekly/" "$filename"
  done < <(find "$backup_dir/daily/" -mindepth 1 -maxdepth 1 -type d -printf "%Tw\t%p${IFS}" | grep -P '^('$weekly_day')' | cut -f 2- 2>/dev/null)
  ...
  #rm -Rf  $verbose $(find "$backup_dir/weekly/" -mindepth 1 -maxdepth 1 -type d -printf "%Tw\t%p${IFS}" | grep -Pv '^('$weekly_day')' | cut -f 2- 2>/dev/null)
  # Delete monthly backups excepted the first day of the month
  #rm -Rf  $verbose $(find "$backup_dir/monthly/" -mindepth 1 -maxdepth 1 -type d -printf "%Td\t%p${IFS}" | grep -Pv '^('$monthly_day')' | cut -f 2- 2>/dev/null)
}
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
sync_backups()
{
...
EOF
  ...
  case method in
    ...
    rsync)
      ...
      ;;
    *|ftp )
      [[ ! $(sudo which lftp) ]] && package_cp install lftp
      lftp -f <<EOF
open $host
user $user $pass
lcd $source
...
$global_help
EOF
  [[ $# != 2 ]] && echo "${IFS}${help}${IFS}"  >&2 && exit $EX_DATAERR
...
usb_backup()
{
  ...
  local list=$(find /media -mindepth 1 -maxdepth 1 -path "/media/usb*" -not -empty -type d | sort 2>/dev/null)
...
{
  sudo editor ${config_path}backup-manager.conf
}
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
