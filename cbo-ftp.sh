...
install_proftpd()
{
  ...
  #[ sudo aptitude search '~i ^(proftpd)$' ] || __package_cp -u install proftpd
  #[ sudo aptitude search '~i ^(ucf)$' ] || __package_cp -u install ucf
...
config_proftpd()
{
  sudo editor ${config_path}proftpd/proftpd.conf
