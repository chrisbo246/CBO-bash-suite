...
EOF
  ...
  done < <(find . -type f -regex "$regex" 2>/dev/null)
  beautify_bash_script "$filename"
}
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
beautify_bash_script()
{
  beautifier=$(find /usr/local -type f -name beautify_bash.py 2>/dev/null)
  if [ -f "$beautifier" ]; then
    [ -z "$(sudo which python)" ] && __package_cp install python
  else
...
beautify_xml_script()
{
  [[ ! $(sudo which tidy) ]] && __package_cp install tidy
  tidy -xml -i -m $@
}
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
clean_css()
