...
changeHostName()
{
  sudo editor ${config_path}hostname
}
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
getFQDN()
{
...
EOF
  sudo editor ${config_path}wpa_supplicant/wpa_supplicant.conf
...
get_public_ip ()
{
  ...
  #[ -z "$(sudo which curl)" ] && __package_cp -y install curl
...
get_ip_nat()
{
  ...
  then
    ...
    if=$1   ## specify sudo which interface, e.g. eth0, fxp0
    system=$(uname)
    case $system in
      FreeBSD) sep="inet " ;;
      Linux) sep="addr:" ;;
