...
#   By default, a secure sudo chmod will be apply to files, directories,
#   .htaccess, php.ini, php.cgi, php5.cgi, etc... so add only specific tasks.
#   - sudo chmod specific directories
...
#   - Use symbolic mode sudo chmod with $httpd_users"+w" for files that should be writable by CMS
#   sudo chmod "[ugo][+-=][rwx],ug+w"
...
# 0 = No (default)
# 1 = Will sudo chmod +x *.php and replace "localhost" by "127.0.0.1"
sbox_ausf_mode=0
# Predefined profils
admin_panel=
case $admin_panel in
...
__find_websites()
{
  ...
  case $1 in
    colabtive )
      list=$(find $find_options $find_from_path -type f -regex "$website_directory_pattern" -path "${find_from_path%/}*/config/standard/*" -name "config.php" -exec $BASH -c 'grep -ls "$db_host" {} | sed "s/\/[^\/]*\/[^\/]*\/[^\/]*$//"' \; 2>/dev/null)
    ;;
    dokuwiki )
      list=$(find $find_options $find_from_path -type f -regex "$website_directory_pattern" -path "${find_from_path%/}*/conf/*" -name "local.php" -exec $BASH -c 'grep -lsi "Dokuwiki" {} | sed "s/\/[^\/]*\/[^\/]*$//"' \; 2>/dev/null)
    ;;
    dolibarr )
      list=$(find $find_options $find_from_path -type f -regex "$website_directory_pattern" -path "${find_from_path%/}*/htdocs/conf/*" -name "conf.php" -exec $BASH -c 'grep -ls "$dolibarr_main_url_root" {} | sed "s/\/[^\/]*\/[^\/]*\/[^\/]*$//"' \; 2>/dev/null)
    ;;
    drupal )
      list=$(find $find_options $find_from_path -type f -regex "$website_directory_pattern" -path "${find_from_path%/}*/sites/*" -name "settings.php" -exec $BASH -c 'grep -ls "$hash_salt" {} | sed "s/\/[^\/]*\/[^\/]*\/[^\/]*$//"' \; 2>/dev/null)
    ;;
    elgg )
      list=$(find $find_options $find_from_path -type f -regex "$website_directory_pattern" -path "${find_from_path%/}*/sites/*" -name "settings.php" -exec $BASH -c 'grep -ls "Elgg.Core" {} | sed "s/\/[^\/]*\/[^\/]*$//"' \; 2>/dev/null)
    ;;
    gallery )
      list=$(find $find_options $find_from_path -type f -regex "$website_directory_pattern" -path "${find_from_path%/}*/application/config/*" -name "config.php" -exec $BASH -c 'grep -ls "gallery" {} | sed "s/\/[^\/]*\/[^\/]*\/[^\/]*$//"' \; 2>/dev/null)
    ;;
    joomla )
      list=$(find $find_options $find_from_path -type f -regex "$website_directory_pattern" -path "${find_from_path%/}*/*" -name "configuration.php" -exec $BASH -c 'grep -ls "JConfig" {} | sed "s/\/[^\/]*$//"' \; 2>/dev/null)
    ;;
    magento )
      list=$(find $find_options $find_from_path -type f -regex "$website_directory_pattern" -path "${find_from_path%/}*/app${config_path}*" -name "local.xml" -exec $BASH -c 'grep -ls "Magento" {} | sed "s/\/[^\/]*\/[^\/]*\/[^\/]*$//"' \; 2>/dev/null)
    ;;
    moodle )
      list=$(find $find_options $find_from_path -type f -regex "$website_directory_pattern" -path "${find_from_path%/}*/*" -name "config.php" -exec $BASH -c 'grep -ls "moodledata" {} | sed "s/\/[^\/]*$//"' \; 2>/dev/null)
    ;;
    oxwall )
      list=$(find $find_options $find_from_path -type f -regex "$website_directory_pattern" -path "${find_from_path%/}*/ow_includes/*" -name "config.php" -exec $BASH -c 'grep -ls "OW_URL_HOME" {} | sed "s/\/[^\/]*\/[^\/]*$//"' \; 2>/dev/null)
    ;;
    opencart )
      list=$(find $find_options $find_from_path -type f -regex "$website_directory_pattern" -path "${find_from_path%/}*/admin/*" -name "config.php" -exec $BASH -c 'grep -ls "HTTP_CATALOG" {} | sed "s/\/[^\/]*\/[^\/]*$//"' \; 2>/dev/null)
    ;;
    prestashop )
      list=$(find $find_options $find_from_path -type f -regex "$website_directory_pattern" -path "${find_from_path%/}*/config/*" -name "settings.inc.php" -exec $BASH -c 'grep -ls "_PS_VERSION_" {} | sed "s/\/[^\/]*\/[^\/]*$//"' \; 2>/dev/null)
    ;;
    seo-panel )
      list=$(find $find_options $find_from_path -type f -regex "$website_directory_pattern" -path "${find_from_path%/}*/config/*" -name "sp-config.php" -exec $BASH -c 'grep -ls "SP_INSTALLED" {} | sed "s/\/[^\/]*\/[^\/]*$//"' \; 2>/dev/null)
    ;;
    wordpress )
      list=$(find $find_options $find_from_path -type f -path "${find_from_path%/}*/*" -name "wp-config.php" -exec $BASH -c 'grep -ls "AUTH_KEY" {} | sed "s/\/[^\/]*$//"' \; 2>/dev/null)
    ;;
    * )
      list=$(find $find_options $find_from_path -maxdepth 1 -type d -regex "$website_directory_pattern" -path "${find_from_path%/}*/*" 2>/dev/null)
      type='unknow'
    ;;
  esac
...
__custom_securisation()
{
  ...
  case $1 in
    'colabtive' )
      ...
      sudo chmod $chmod_options $mod 'config/standard/config.php'
      #mod=$web_process'=rx'
      #sudo chmod $chmod_options -R $mod 'files'
    ;;
    'dokuwiki')
      website_version=$(grep -m 1 -Eos "[$]+updateVersion+\s*=\s*[0-9.]+" 'doku.php' | grep -Pos "([0-9.]*)")
      rm -f 'install.php'
      mod=$web_process'=rx'
      sudo chmod $chmod_options -R $mod 'data'
      sudo chmod $chmod_options -R $mod 'data${TMPDIR}'
      sudo chmod $chmod_options -R $mod 'lib/plugins'
      mod=$public'+rx'
      sudo chmod $chmod_options -R $mod 'lib'
      mod=$config_file_mod','$writable_mod
      sudo chmod $chmod_options $mod 'conf/local.php'
      sudo chmod $chmod_options $mod 'conf/users.auth.php'
      sudo chmod $chmod_options $mod 'conf/acl.auth.php'
      sudo chmod $chmod_options $mod 'conf/plugins.local.php'
    ;;
    'dolibarr' )
      if [ ! -e 'install.lock' ]; then echo '' > 'documents/install.lock'; fi
      mod=$web_process"=r"
      sudo chmod $chmod_options $mod 'documents/install.lock'
      mod=$config_file_mod','$writable_mod
      sudo chmod $chmod_options $mod 'htdocs/conf/conf.php'
    ;;
    'drupal' )
      ...
      #mod=$web_process'=rx'
      #find $find_options . -type d -path "*/sites/*/files/*" -exec sudo chmod $chmod_options -R $mod {} \;
      mod=$config_file_mod','$writable_mod
      find $find_options . -type f -path "*/sites/*" -name "settings.php" -exec sudo chmod $chmod_options $mod {} \;
...
EOF
    ...
    'elgg' )
      ...
      mod=$config_file_mod','$writable_mod
      sudo chmod $chmod_options $mod 'engine/settings.php'
read -d '' php_flags <<EOF
safe_mode = Off
register_globals = Off
EOF
    ...
    'gallery' )
      #mod=$public'=rx'
      #sudo chmod $chmod_options $mod 'install'
      #sudo chmod $chmod_options $mod 'upgrade'
    ;;
    'joomla' )
      #string=$(find $find_options . -path "*/libraries/*" -type f -name version.php -exec grep -Pos '[$]+REL\EASE\s*=\s*[\'"]+[0-9.]+[\'"]+' "{}" \; 2>/dev/null)
      #website_version=$(echo "$string" | head -n 1 | grep -Pos "([0-9.]*)")
      rm -rf 'installation'
      #mod=$public'=r'
      #find $find_options . -type f -exec sudo chmod $chmod_options $mod {} \;
      #mod=$public'=rx'
      #find $find_options . -type d -exec sudo chmod $chmod_options $mod {} \;
      mod=$config_file_mod','$writable_mod
      sudo chmod $chmod_options $mod 'configuration.php'
      mod=$writable_mod
      sudo chmod $chmod_options $mod 'administrator/components'
      find $find_options 'administrator/language' -type d -exec sudo chmod $chmod_options $mod {} \;
      find $find_options 'administrator/manifests' -type d -exec sudo chmod $chmod_options $mod {} \;
      sudo chmod $chmod_options $mod 'administrator/modules'
      sudo chmod $chmod_options $mod 'administrator/templates'
      sudo chmod $chmod_options $mod 'components'
      find $find_options 'images' -type d -exec sudo chmod $chmod_options $mod {} \;
      find $find_options 'language' -type d -exec sudo chmod $chmod_options $mod {} \;
      sudo chmod $chmod_options $mod 'libraries'
      sudo chmod $chmod_options $mod 'media'
      sudo chmod $chmod_options $mod 'modules'
      sudo chmod $chmod_options $mod 'plugins'
      find $find_options 'plugins' -maxdepth 1 -type d -exec sudo chmod $chmod_options $mod {} \;
      sudo chmod $chmod_options $mod 'templates'
      sudo chmod $chmod_options $mod 'cache'
      sudo chmod $chmod_options $mod 'administrator/cache'
      sudo chmod $chmod_options $mod 'logs'
      sudo chmod $chmod_options $mod 'tmp'
      # J1.5
      sudo chmod $chmod_options $mod 'plugins/content'
      sudo chmod $chmod_options $mod 'plugins/editors'
      sudo chmod $chmod_options $mod 'plugins/editors-xtd'
      sudo chmod $chmod_options $mod 'plugins/search'
      sudo chmod $chmod_options $mod 'plugins/system'
      sudo chmod $chmod_options $mod 'plugins/user'
      sudo chmod $chmod_options $mod 'plugins/xmlrpc'
      #sudo chmod $chmod_options -R $mod 'images/stories'
      #sudo chmod $chmod_options $mod 'images/banners'
      #sudo chmod $chmod_options $mod 'administrator/backups'
      #sudo chmod $chmod_options $mod 'language/pdf_fonts'
read -d '' php_flags <<EOF
safe_mode = Off
display_errors = Off
file_uploads = On;
...
EOF
    ...
    'moodle' )
      ...
      #mod=$web_process'=rx'
      #sudo chmod $chmod_options -R $mod 'moodledata'
    ;;
    'oxwall' )
      website_version=$(grep -m 1 -Eos "<version>.*<\/version>" 'ow_version.xml' | grep -Pos "([0-9.]*)")
      mod=$config_file_mod','$writable_mod
      sudo chmod $chmod_options $mod 'ow_includes/config.php'
      mod=$writable_mod
      sudo chmod $chmod_options -R $mod 'ow_logs/*'
      sudo chmod $chmod_options -R $mod 'ow_pluginfiles/*'
      sudo chmod $chmod_options -R $mod 'ow_plugins/*'
      sudo chmod $chmod_options -R $mod 'ow_static/*'
      sudo chmod $chmod_options -R $mod 'ow_smarty/template_c/*'
      sudo chmod $chmod_options -R $mod 'ow_themes/*'
      sudo chmod $chmod_options -R $mod 'ow_updates/*'
      sudo chmod $chmod_options -R $mod 'ow_userfiles/*'
read -d '' php_flags <<EOF
register_globals = Off
safe_mode = Off
suPHP = Off
...
EOF
    ...
    'prestashop' )
      ...
      #mv admin admin12345
      mod=$writable_mod
      sudo chmod $chmod_options $mod -R 'config'
      sudo chmod $chmod_options $mod -R 'cache'
      sudo chmod $chmod_options $mod -R 'log'
      sudo chmod $chmod_options $mod -R 'img'
      sudo chmod $chmod_options $mod -R 'mails'
      sudo chmod $chmod_options $mod -R 'modules'
      sudo chmod $chmod_options $mod -R 'themes/default/lang'
      sudo chmod $chmod_options $mod -R 'themes/default/pdf/lang'
      sudo chmod $chmod_options $mod -R 'themes/default/cache'
      sudo chmod $chmod_options $mod -R 'translations'
      sudo chmod $chmod_options $mod -R 'upload'
      sudo chmod $chmod_options $mod -R 'download'
      sudo chmod $chmod_options $mod 'sitemap.xml'
read -d '' php_flags <<EOF
...
EOF
    ...
    'seo-panel' )
      ...
      rm -rf 'install'
      mod=$config_file_mod','$writable_mod
      sudo chmod $chmod_options $mod 'config/sp-config.php'
      mod=$writable_mod
      sudo chmod $chmod_options -R $mod 'tmp'
      #mod=$web_process'+x'
      #sudo chmod $chmod_options $mod 'cron.php'
      #sudo chmod $chmod_options $mod 'siteauditorcron.php'
      #sudo chmod $chmod_options $mod 'directorycheckercron.php'
read -d '' php_flags <<EOF
allow_url_fopen = On
short_open_tag = On
EOF
    ...
    'wordpress' )
      ...
      mod=$writable_mod
      sudo chmod $chmod_options -R $mod 'wp-content'
      mod=$config_file_mod','$writable_mod
      sudo chmod $chmod_options $mod 'wp-config.php'
      mod=$writable_mod
      sudo chmod $chmod_options $mod '.htaccess'
      #mod='u+x'
      #sudo chmod $chmod_options $mod 'wp-cron.php'
read -d '' php_flags <<EOF
file_uploads = On
EOF
      __set_php_config "php_flag" "$php_flags" ".htaccess"
...
EOF
  ...
  find $find_options . -type f -name ".htaccess" -exec sudo chmod $chmod_options $mod {} \;
  mod=$public'='
  [[ ! $quiet ]] && echo "Apply $mod permissions to .htpasswd files"
  find $find_options . -type f -name ".htpasswd" -exec sudo chmod $chmod_options $mod {} \;
  mod=$public'='
  [[ ! $quiet ]] && echo "Apply $mod permissions to php.ini files"
  find $find_options . -type f -name "php.ini" -exec sudo chmod $chmod_options $mod {} \;
  mod=$public'='
  [[ ! $quiet ]] && echo "Apply $mod permissions to php and other server-side scripts"
  find $find_options . -type f -regextype posix-extended -regex ".*\.("$server_side_scripts")[0-9]?$" -exec sudo chmod $chmod_options $mod {} \;
  mod=$web_process'+x'
  [[ ! $quiet ]] && echo "Apply $mod permissions to cgi files"
  find $find_options . -type f -regextype posix-extended -regex ".*\.(cgi)$" -exec sudo chmod $chmod_options $mod {} \;
  if [ $sbox_ausf_mode -eq 1 ]; then
    mod=$public'+x'
    [[ ! $quiet ]] && echo "Apply patch for sbox_ausf_mode"
    #find $find_options . -type f -regextype posix-extended -regex ".*/index\.(php|htm|html|asp)+[0-9]?$" -exec sudo chmod $chmod_options $mod {} \;
    find . -iname '*.php' -exec sudo chmod $chmod_options $mod {} \;
  ...
  if [ $allow_http_updates -eq 1  ]; then
    ...
    sudo chmod -R $chmod_options $mod * .*
  fi
...
[[ ! $(declare -Ff include_once) ]] && . "${BASH_SOURCE%/*}/cbo.sh"
#COLLABTIVE
...
# sudo chmod 707 images
# sudo chmod 707 images/stories
