...
install_ispconfig()
{
  ...
  sudo sed -r -i 's|(127.0.0.1[ \t]+)(localhost)[ \t]*$|\1localhost.localdomain\t\2|' "${config_path}hosts"
  fqdn=$(hostname -f)
  ip=$(hostname -i)
  sudo sed -r -i "s|^[ \t1-9\.\:\/]+([ \t]*$fqdn)|$ip\1|" "${config_path}hosts"
  sudo echo "$(hostname -f)" > "${config_path}hostname"
  ${service_path}hostname.sh
  ...
  sudo sudo sed -r -i "s|^([ \t]*(deb\|deb-src).*debian.org/debian/?[ \t]+[^ \t]+).*|\1 main contrib non-free|" "${config_path}apt/sources.list"
  sudo editor ${config_path}apt/sources.list
  __package_cp update
  ...
  #echo "set postfix/destinations server1.domain.com, localhost.domain.com, , localhost"  | debconf-communicate
  ...
  # Auto-configuration
  sudo sed -r -i "s|#([ \t]*(submission\|smtps)[ \t]+inet)|\1|" "${config_path}postfix/master.cf"
  sudo sed -r -i "s|#([ \t]*-o[ \t]+(syslog_name\|smtpd_tls_security_level\|smtpd_tls_wrappermode\|smtpd_sasl_auth_enable\|smtpd_client_restrictions))|\1|" "${config_path}postfix/master.cf"
  sudo sed -r -i "s|^([ \t]*bind-address[ \t]+=)|#\1|" "${config_path}mysql/my.cnf"
  sudo sed -r -i "s|^([ \t]*</FilesMatch>)|\1${IFS}\tAddType application/x-httpd-suphp .php .php3 .php4 .php5 .phtml|" "${config_path}apache2/mods-available/suphp.conf"
  sudo sed -r -i "s|^([ \t]*<FilesMatch)|#\1|" "${config_path}apache2/mods-available/suphp.conf"
  sudo sed -r -i "s|^([ \t]*SetHandler application/x-httpd-suphp)|#\1|" "${config_path}apache2/mods-available/suphp.conf"
  sudo sed -r -i "s|^([ \t]*</FilesMatch>)|#\1|" "${config_path}apache2/mods-available/suphp.conf"
  sudo sed -r -i "s|^([ \t]*application/x-ruby)|#\1|" "${config_path}mime.types"
...
EOF
  sudo sed -r -i "s|^([ \t]*STANDALONE_OR_INETD[ \t]*=[ \t]*).*$|\1standalone|" "${config_path}default/pure-ftpd-common"
  sudo sed -r -i "s|^([ \t]*VIRTUALCHROOT[ \t]*=[ \t]*).*$|\1true|" "${config_path}default/pure-ftpd-common"
  sudo sed -r -i "s|^([ \t]*[^ \t]+[ \t]+/[ \t]+[^ \t]+[ \t]+[^ \t]+)|\1,usrjquota=quota.user,grpjquota=quota.group,jqfmt=vfsv0|" "${config_path}fstab"
  sudo sed -r -i "s|^([ \t]*[^#]+)|#\1|" "${config_path}cron.d/awstats"
...
EOF
  sudo sed -r -i "s|<Directory /usr/share/squirrelmail>.*<\Directory>|$content|" "${config_path}apache2/conf.d/squirrelmail.conf"
  ...
  sudo chmod 600 ${config_path}ssl/private/pure-ftpd.pem
  mount -o remount /
  quotacheck -avugmf
  quotaon -avug
  ...
  filename=$(find ${config_path}php* -type f -name xcache.ini 2>/dev/null)
  [ -e "$filename" ] && sudo sed -r -i "s|(xcache\.var_size[ \t]*=[ \t]*)[0a-zA-Z]+|\116M|" "$filename"

  # Fix bastille-firewall
  [[ $(sudo which bastille-netfilter) ]] && mkdir -p /var/lock/subsys/bastille-firewall

  # Fix amavis
  [[ $(sudo which amavisd-release) ]] && chown -R amavis:amavis /var/run/amavis
...
generate_ispconfig_certificat()
{
  ...
  sudo chmod 400 /root/ispconfig/httpd/conf/ssl.key/server.key
