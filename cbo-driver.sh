...
EOF
  ...
  sudo editor ${config_path}X11/xorg.conf
...
install_printer ()
{
  ...
  # sudo editor ${config_path}udev/rules.d/45-libsane.rules
...
install_ppc ()
{
  ...
  gksudo sudo editor ${config_path}modprobe.d/synce     # Créer alors le fichier suivant:
  ...
  sudo editor ${config_path}udev/rules.d/10-ipaq.rules    # Copier/Coller le script suivant:
