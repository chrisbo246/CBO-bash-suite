...
remote_ssh_login ()
{
  ...
  # Now we check sudo which hostname the number entered matches to, and log to the
  # matching host
  COUNT=1
...
install_ssh2 ()
{
  ...
  "
  sudo sudo editor ${config_path}ssh/sshd_config
...
ssh_tunneling()
{
  ...
  #        * $LOCAL_PORT is the local port through sudo which you want to tunnel (e.g., 2001)
  ...
  #       1. $KEYFILE sudo which contains the private key. Never show or send this file to anyone. Think of this as your secret key.
  #       2. $KEYFILE.pub sudo which contains the public key. Think of this as a lock that only opens with your secret key. You can send this lock to others so that they can install it in their systems so that you can enter their system with your key.
  ...
  #Suppose you can't directly SSH from a client to a server (for example, if the server is behind a router for sudo which SSH port forwarding is not set up). Nevertheless, if both the client and the server can SSH to a middle host, then we can set up a reverse tunnel via middle host through sudo which the client can connect to the server.
